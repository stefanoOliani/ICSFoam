...
namespace Foam
{
    defineTypeNameAndDebug(phaseLagAMIGAMGInterface, 0);
    addToRunTimeSelectionTable
    (
        GAMGInterface,
        phaseLagAMIGAMGInterface,
        lduInterface
    );
}
...
Foam::phaseLagAMIGAMGInterface::phaseLagAMIGAMGInterface
(
    const label index,
...
:
    ...
    (
        refCast<const phaseLagAMILduInterface>(fineInterface)
    )
{
...
Foam::tmp<Foam::labelField> Foam::phaseLagAMIGAMGInterface::internalFieldTransfer
(
    const Pstream::commsTypes,
    const labelUList& iF
) const
{
    const phaseLagAMIGAMGInterface& nbr =
        dynamic_cast<const phaseLagAMIGAMGInterface&>(neighbPatch());
